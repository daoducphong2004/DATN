...
@section('title')
    Thảo luận - Cổng Light Novel - Đọc Light Novel
@endsection
@section('content')
    <style>
        .cricle_forum {
            ...
            <section class="board-toolkit clear">
                    ...
                    <select name="category" id="category" class="board_categ-list" onchange="this.form.submit()">
                        ...
                        @foreach ($categories as $item_categories)
                            <option value="{{ $item_categories->id }}"
                                {{ request('category') == $item_categories->id ? 'selected' : '' }}>
                ...
                @if (Auth::check())
                    <a class="button button-newpost button-green" href="{{ route('themthaoluan') }}"><i
                        ...
                        Thêm</a>
                @else
                    <p style="margin-top:10px"><strong>Bạn phải <a href="{{ route('login') }}" style="color: red">đăng
                                nhập</a> để tạo thảo luận</strong></p>
                @endif
